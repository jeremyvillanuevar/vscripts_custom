...
//
//=============================================================================
//2 problems, it seems that the array doesnt generate well, and the delegate isnt working
...
DirectorOptions <-
{
	local finaleStageList = []; 
	local OnChangeFinaleMusic	 = ""
...
function GetDirectorOptions()
{
	...
	if ( DirectorScript.MapScript.rawin( "DirectorOptions") )
	{
		if ( result != null )
		{
				result.setdelegate(DirectorScript.MapScript.DirectorOptions);
		}
	...
	if ( DirectorScript.MapScript.LocalScript.rawin( "DirectorOptions") )
	{
		if ( result != null )
		{
			result.setdelegate(DirectorScript.MapScript.LocalScript.DirectorOptions);
		}
	...
	if ( DirectorScript.ChallengeScript.rawin( "DirectorOptions") )
	{
		if ( result != null )
		{
			result.setdelegate(getroottable().DirectorScript.ChallengeScript.DirectorOptions);
		}
